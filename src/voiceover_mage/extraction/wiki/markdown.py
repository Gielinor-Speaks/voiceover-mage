# ABOUTME: Markdown-only NPC extractor using Crawl4AI
...
# Type protocol for Crawl4AI result
...
class MarkdownNPCExtractor(RawNPCExtractor):
    ...
    This is Phase 1 of the extraction pipeline - simple, fast extraction.
    """

    def __init__(self, headless: bool = True, client: httpx.AsyncClient | None = None):
        ...
        try:
            with suppress_library_output():
                async with AsyncWebCrawler(config=browser_cfg) as crawler:
                    result = cast(CrawlResult, await crawler.arun(url=url, config=crawl_config))
    ...
    async def _extract_image_urls(
        self, markdown: str, npc_name: str, npc_variant: str | None = None
    ...
    async def _extract_simple_image_urls(self, markdown: str) -> tuple[str | None, str | None]:
        ...
        try:
            ...
            self.logger.debug("Regex image extraction", has_chathead=bool(chathead_url), has_main=bool(image_url))
