...
class TrackedField[T](BaseModel):
    ...
    confidence: float = Field(
        ge=0.0, le=1.0,
        description="Confidence level of the value, from 0.0 (no confidence) to 1.0 (certain)"
    )
    evidence: str | None = Field(
        None,
        description="Text snippet or reasoning that supports this value"
    )
...
class NPCWikiSourcedData(BaseModel):
    ...
    gender: TrackedField[Literal['male', 'female', 'unknown']] = Field(
        ..., 
        description="The gender of the NPC"
    )
    race: TrackedField[str] = Field(
        ...,
        description="The race/species of the NPC (human, elf, dwarf, tzhaar, etc.)"
    )
    ...
    social_class: TrackedField[
        ...
        ...,
        description="Social standing in society"
    )
    ...
    accent_region: TrackedField[str] = Field(
        ..., 
        description="Regional accent or dialect indicators"
    )
    
    # Capabilities and skills
    combat_experience: TrackedField[Literal['veteran_warrior', 'experienced', 'some_training', 'civilian', 'pacifist']] = Field(
        ..., 
        description="Level of combat experience"
    )
    ...
    speech_formality: TrackedField[Literal['archaic_formal', 'formal', 'professional', 'casual', 'rough', 'crude']] = Field(
        ..., 
        description="How formally they speak"
    )
    ...
    relationships: list[TrackedField[str]] = Field(
        ..., 
        description="Important relationships with other NPCs"
    )
    
    # Dialogue and quotes
    dialogue_examples: list[str] = Field(
        default_factory=lambda: [], 
        description="Actual quotes from the NPC if available"
    )
    common_phrases: list[TrackedField[str]] = Field(
        default_factory=lambda: [],
        description="Recurring phrases or expressions they use"
    )
