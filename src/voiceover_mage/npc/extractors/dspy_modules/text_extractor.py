...
class NPCTextCharacteristics(BaseModel):
    ...
    personality_traits: str = Field(
        default="",
        description="Core personality traits description (e.g., 'gruff but fair-minded, with a deep wisdom earned through years of hardship')"
    )
    occupation: str = Field(
        default="",
        description="NPC's role or job (e.g., 'blacksmith', 'quest giver', 'shopkeeper')"
    )
    social_role: str = Field(
        default="",
        description="Social position or archetype (e.g., 'village elder', 'mysterious stranger')"
    )
    dialogue_patterns: str = Field(
        default="",
        description="Speech patterns and mannerisms description (e.g., 'speaks in formal, archaic tones with occasional lapses into colloquial expressions when excited')"
    )
    emotional_range: str = Field(
        default="",
        description="Range of emotions expressed (e.g., 'generally calm and measured, but prone to sudden bursts of passionate enthusiasm')"
    )
    background_lore: str = Field(
        default="",
        description="Key background information and lore context"
    )
    ...
    reasoning: str = Field(
        default="",
        description="Explanation of how characteristics were identified from the text"
    )
...
class TextExtractionSignature(dspy.Signature):
    ...
    markdown_content: str = dspy.InputField(
        description="Raw markdown content from the NPC's wiki page"
    )
    npc_name: str = dspy.InputField(
        description="Name of the NPC to extract characteristics for"
    )
    ...
    personality_traits: str = dspy.OutputField(
        description="Descriptive summary of core personality traits (e.g., 'wise and patient mentor with a dry sense of humor and fierce loyalty to friends')"
    )
    occupation: str = dspy.OutputField(
        description="The NPC's primary role or occupation"
    )
    social_role: str = dspy.OutputField(
        description="Social position or archetype within the game world"
    )
    dialogue_patterns: str = dspy.OutputField(
        description="Descriptive summary of speech patterns and mannerisms (e.g., 'speaks with measured cadence, often pausing for effect, uses metaphors from nature')"
    )
    emotional_range: str = dspy.OutputField(
        description="Descriptive summary of emotional range and expressions (e.g., 'typically serene but capable of righteous anger, shows warmth through subtle gestures')"
    )
    background_lore: str = dspy.OutputField(
        description="Summary of key background information and context"
    )
    confidence: float = dspy.OutputField(
        description="Confidence score from 0.0 to 1.0 for the extraction accuracy"
    )
    reasoning: str = dspy.OutputField(
        description="Step-by-step reasoning for characteristic identification"
    )
