...
class ProgressLogInterceptor(logging.Handler):
    ...
    def __init__(self, progress_callback: Callable[[str, dict[str, Any]], None]):
        ...
        self.status_patterns = {
            "lookup": {"pattern": r"Looking up NPC page URL", "message": "🔍 Finding NPC page...", "stage": "lookup"},
            "url_retrieved": {
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        try:
            ...
            if (
                ...
                and any(
                    keyword in message.lower() for keyword in ["start", "processing", "extract", "crawl", "configur"]
                )
            ):
                self.progress_callback(
                    f"🔄 {message[:50]}...",
